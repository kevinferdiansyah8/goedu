...
<div class="container mx-auto px-4 py-6" x-data="{ selectedEvent: null, showModal: false }">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group flex flex-col justify-between">
            <div>
                <div class="h-48 bg-gray-200 relative overflow-hidden">
                    ...
                    <span class="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-blue-600">
                        {{ $event->status === 'Dipublikasikan' ? 'Aktif' : 'Draft' }}
                    </span>
                </div>
                <div class="p-5">
                    <div class="flex items-center gap-2 text-xs text-blue-600 font-medium mb-2">
                        <i data-lucide="calendar" class="w-3 h-3"></i> {{ \Carbon\Carbon::parse($event->tanggal_pelaksanaan)->translatedFormat('d F Y') }}
                    </div>
                    <h3 class="text-lg font-bold text-gray-800 mb-2 group-hover:text-blue-600 transition-colors line-clamp-1">{{ $event->judul }}</h3>
                    <p class="text-gray-600 text-sm mb-4 line-clamp-3">{{ $event->deskripsi }}</p>
                </div>
            </div>
            <div class="p-5 pt-0">
                <button @click="selectedEvent = @json($event); showModal = true" class="text-blue-600 font-medium text-sm flex items-center gap-1 hover:gap-2 transition-all">

...
<div class="max-w-7xl mx-auto" x-data="raporApp()" x-init="init()">
    ...
    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-5 mb-6">
        ...
        <div class="flex flex-wrap items-end gap-3">
            <div class="flex-1 min-w-[180px]">
                ...
                <select x-model="selectedKelas" @change="filterSiswa()"
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-100 focus:border-orange-400 transition-all">
            ...
            <button @click="tampilkanRapor()"
                    class="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-orange-500 to-rose-600 hover:from-orange-600 hover:to-rose-700 text-white rounded-xl font-bold text-sm shadow-lg shadow-orange-200 transition-all active:scale-95">
    ...
    <div x-show="!showRapor" class="bg-white border border-gray-100 rounded-2xl shadow-sm p-12 text-center">
        ...
        @if($dataSiswa->count() === 0)
            <h3 class="text-lg font-bold text-gray-400 mb-1">Belum Ada Data Nilai</h3>
            <p class="text-sm text-gray-400">Data rapor akan tampil setelah guru mengisi nilai siswa.</p>
        @else
                    ...
                    <tbody class="divide-y divide-gray-50">
                        <template x-for="(n, idx) in currentSiswa?.nilai || []" :key="idx">
                            <tr class="hover:bg-orange-50/20 transition-colors">
                                ...
                                <td class="px-5 py-3 text-center">
                                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-xl text-sm font-extrabold"
                                          :class="n.rata >= 85 ? 'bg-emerald-100 text-emerald-700' : (n.rata >= 75 ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-600')"
                                          x-text="n.rata"></span>
                                </td>
                                <td class="px-5 py-3 text-center">

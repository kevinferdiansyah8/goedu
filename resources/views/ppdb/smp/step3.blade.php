...
@section('title', 'Pendaftaran SMP - Step 3 | Dokumen')
@section('content')
...
<section class="min-h-[calc(100vh-120px)] flex items-center justify-center py-10 px-4">
  <div class="w-full max-w-xl">
    ...
    <div class="flex items-center justify-center mb-8">
      ...
      <div class="w-16 h-0.5 bg-green-300 mb-5"></div>
      <div class="flex flex-col items-center">
        <div class="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold text-sm shadow-lg shadow-indigo-200 ring-4 ring-indigo-100">3</div>
        <span class="text-xs font-semibold text-indigo-600 mt-1.5">Dokumen</span>
      </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-gray-100/80 border border-gray-100 overflow-hidden">
      ...
      <div class="bg-gradient-to-r from-indigo-600 to-indigo-500 px-8 py-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-indigo-100 text-xs font-medium uppercase tracking-widest">Pendaftaran SMP • Langkah 3 dari 3</p>
            <h1 class="text-white font-extrabold text-lg">Kelengkapan Dokumen SMP</h1>
          </div>
        </div>
      ...
      {{-- Body --}}
      <div class="px-8 py-8">
        <form action="{{ url('/ppdb/register/smp/success') }}" method="get" class="space-y-5">
          ...
          <div class="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3.5 flex gap-2.5">
            ...
            <p class="text-xs text-amber-700 leading-relaxed">
              Siapkan dokumen fisik berikut sebelum hari H pendaftaran. Centang semua dokumen yang sudah disiapkan.
            </p>
          </div>

          {{-- Checklist Dokumen SMP --}}
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Dokumen Wajib SMP <span class="text-red-400">*</span></label>
            <div class="space-y-2.5">
              @php
                $dokumen = [
                  ['icon' => 'scroll-text',  'label' => 'Ijazah SD / MI (asli + fotokopi)'],
                  ['icon' => 'file-check',   'label' => 'SKHUN (Surat Keterangan Hasil Ujian Nasional) SD'],
                  ['icon' => 'file-badge',   'label' => 'Akta Kelahiran (asli + fotokopi)'],
                  ['icon' => 'id-card',      'label' => 'Kartu Keluarga (asli + fotokopi)'],
                  ['icon' => 'book-open',    'label' => 'Rapor SD kelas 4–6 (fotokopi)'],
                  ['icon' => 'image',        'label' => 'Pas Foto 3×4 berwarna (2 lembar)'],
                ];
              @endphp
              @foreach($dokumen as $dok)
              <label class="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 cursor-pointer hover:border-indigo-300 hover:bg-indigo-50 transition-all has-[:checked]:border-indigo-400 has-[:checked]:bg-indigo-50/60 group">
                <input type="checkbox" name="dokumen[]" value="{{ $dok['label'] }}" class="w-4 h-4 accent-indigo-600 shrink-0">
                <i data-lucide="{{ $dok['icon'] }}" class="w-4 h-4 text-gray-400 group-has-[:checked]:text-indigo-500 shrink-0 transition-colors"></i>
                <span class="text-sm text-gray-700 group-has-[:checked]:text-indigo-700 font-medium transition-colors">{{ $dok['label'] }}</span>
              </label>
              @endforeach
            </div>
          </div>
          ...
          <div>
            ...
            <div class="space-y-2.5">
              <label class="flex items-center gap-3 px-4 py-3 rounded-xl border border-blue-100 bg-blue-50/50 cursor-pointer hover:border-blue-300 transition-all has-[:checked]:border-blue-400 has-[:checked]:bg-blue-50 group">
                <input type="checkbox" name="dokumen[]" value="Bukti alamat (tagihan listrik/air)" class="w-4 h-4 accent-blue-600 shrink-0">
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Dokumen Tambahan — Jalur Perpindahan</label>
            <div class="space-y-2.5">
              <label class="flex items-center gap-3 px-4 py-3 rounded-xl border border-orange-100 bg-orange-50/50 cursor-pointer hover:border-orange-300 transition-all has-[:checked]:border-orange-400 has-[:checked]:bg-orange-50 group">
                <input type="checkbox" name="dokumen[]" value="Surat tugas pindah orang tua" class="w-4 h-4 accent-orange-500 shrink-0">
                <i data-lucide="file-check-2" class="w-4 h-4 text-orange-300 group-has-[:checked]:text-orange-500 shrink-0"></i>
                <span class="text-sm text-gray-700 font-medium">Surat Keputusan / Tugas Pindah Orang Tua</span>
              </label>
            </div>
          </div>
          @endif
          ...
          <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5">
            <label class="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" name="setuju" required class="w-4 h-4 accent-indigo-600 mt-0.5 shrink-0">
          ...
          <div class="flex gap-3">
            <a href="{{ url('/ppdb/register/smp/step2?jalur='.request('jalur')) }}"

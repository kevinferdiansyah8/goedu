...
@section('title', 'Pendaftaran SD - Step 3 | Dokumen')
@section('content')
...
<section class="min-h-[calc(100vh-120px)] flex items-center justify-center py-10 px-4">
  <div class="w-full max-w-xl">
    ...
    <div class="flex items-center justify-center mb-8">
      ...
      <div class="w-16 h-0.5 bg-green-300 mb-5"></div>
      <div class="flex flex-col items-center">
        <div class="w-10 h-10 rounded-full bg-red-500 text-white flex items-center justify-center font-bold text-sm shadow-lg shadow-red-200 ring-4 ring-red-100">3</div>
        <span class="text-xs font-semibold text-red-500 mt-1.5">Dokumen</span>
      </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-gray-100/80 border border-gray-100 overflow-hidden">
      ...
      <div class="bg-gradient-to-r from-red-500 to-red-400 px-8 py-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-red-100 text-xs font-medium uppercase tracking-widest">Pendaftaran SD • Langkah 3 dari 3</p>
            <h1 class="text-white font-extrabold text-lg">Kelengkapan Dokumen SD</h1>
          </div>
        </div>
      ...
      {{-- Body --}}
      <div class="px-8 py-8">
        <form action="{{ url('/ppdb/register/sd/success') }}" method="get" class="space-y-5">
            ...
            <div class="space-y-2.5">
              @php
                $dokumen = [
                  ...
                  ['icon' => 'scroll-text',    'label' => 'Sertifikat / Ijazah TK atau PAUD'],
                  ['icon' => 'file-check',     'label' => 'Surat Keterangan Lulus TK (jika ada)'],
                  ['icon' => 'map-pin',        'label' => 'Surat Keterangan Domisili (jika alamat beda KK)'],
                  ['icon' => 'image',          'label' => 'Pas Foto 3×4 berwarna (2 lembar)'],
                ];
              @endphp
              @foreach($dokumen as $dok)
              <label class="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 cursor-pointer hover:border-red-300 hover:bg-red-50 transition-all has-[:checked]:border-red-400 has-[:checked]:bg-red-50/60 group">
                <input type="checkbox" name="dokumen[]" value="{{ $dok['label'] }}" class="w-4 h-4 accent-red-500 shrink-0">
                <i data-lucide="{{ $dok['icon'] }}" class="w-4 h-4 text-gray-400 group-has-[:checked]:text-red-500 shrink-0 transition-colors"></i>
                <span class="text-sm text-gray-700 group-has-[:checked]:text-red-700 font-medium transition-colors">{{ $dok['label'] }}</span>
              </label>
              @endforeach
            </div>
          </div>

          {{-- Dokumen Jalur Zonasi Khusus --}}
          ...
          <div>
            ...
            <div class="space-y-2.5">
              <label class="flex items-center gap-3 px-4 py-3 rounded-xl border border-green-100 bg-green-50/50 cursor-pointer hover:border-green-300 transition-all has-[:checked]:border-green-400 has-[:checked]:bg-green-50 group">
                <input type="checkbox" name="dokumen[]" value="Kartu Indonesia Pintar (KIP)" class="w-4 h-4 accent-green-600 shrink-0">
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">
              Dokumen Tambahan — Jalur Perpindahan
            </label>
            <div class="space-y-2.5">
          ...
          <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5">
            <label class="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" name="setuju" required class="w-4 h-4 accent-red-500 mt-0.5 shrink-0">
          ...
          <div class="flex gap-3">
            <a href="{{ url('/ppdb/register/sd/step2?jalur='.request('jalur')) }}"

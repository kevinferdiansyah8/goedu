...
@section('title', 'Pendaftaran SMA - Step 3 | Dokumen')
@section('content')
...
<section class="min-h-[calc(100vh-120px)] flex items-center justify-center py-10 px-4">
  <div class="w-full max-w-xl">
    ...
    <div class="flex items-center justify-center mb-8">
      ...
      <div class="w-16 h-0.5 bg-green-300 mb-5"></div>
      <div class="flex flex-col items-center">
        <div class="w-10 h-10 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-sm shadow-lg shadow-emerald-200 ring-4 ring-emerald-100">3</div>
        <span class="text-xs font-semibold text-emerald-600 mt-1.5">Dokumen</span>
      </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-gray-100/80 border border-gray-100 overflow-hidden">
      <div class="bg-gradient-to-r from-emerald-600 to-emerald-500 px-8 py-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-emerald-100 text-xs font-medium uppercase tracking-widest">Pendaftaran SMA • Langkah 3 dari 3</p>
            <h1 class="text-white font-extrabold text-lg">Kelengkapan Dokumen SMA</h1>
          </div>
        </div>
      ...
      <div class="px-8 py-8">
        <form action="{{ url('/ppdb/register/sma/success') }}" method="get" class="space-y-5">
          ...
          <div class="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3.5 flex gap-2.5">
            ...
            <p class="text-xs text-amber-700 leading-relaxed">Siapkan dokumen fisik berikut sebelum hari H pendaftaran.</p>
          </div>

          {{-- Dokumen Wajib SMA --}}
          <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Dokumen Wajib SMA <span class="text-red-400">*</span></label>
            <div class="space-y-2.5">
              ...
              <label class="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 cursor-pointer hover:border-emerald-300 hover:bg-emerald-50 transition-all has-[:checked]:border-emerald-400 has-[:checked]:bg-emerald-50/60 group">
                <input type="checkbox" name="dokumen[]" value="{{ $dok['label'] }}" class="w-4 h-4 accent-emerald-600 shrink-0">
                <i data-lucide="{{ $dok['icon'] }}" class="w-4 h-4 text-gray-400 group-has-[:checked]:text-emerald-500 shrink-0 transition-colors"></i>
                <span class="text-sm text-gray-700 group-has-[:checked]:text-emerald-700 font-medium transition-colors">{{ $dok['label'] }}</span>
              </label>
              @endforeach
            </div>
          </div>
          ...
          <div class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5">
            <label class="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" name="setuju" required class="w-4 h-4 accent-emerald-600 mt-0.5 shrink-0">
          ...
          <div class="flex gap-3">
            <a href="{{ url('/ppdb/register/sma/step2?jalur='.request('jalur')) }}"
